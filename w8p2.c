...
double convertLbsKg(const double* pounds, double* resultToKg) {
	...
	double poundsToKg = (*pounds) / LBS_TO_KG;
...
int convertLbsG(const double* pounds, int* resultToG) {
	...
	int poundsToG = ((*pounds) / LBS_TO_KG) * 1000;
...
struct ReportData calculateReportData(struct CatFoodInfo info) {
	...
	Data.cost_Per_Serving = calculateCost_Per_Serving(&Data.productPrice, &Data.servings, &Data.cost_Per_Serving);
	Data.caloriesCost_Per_Serving = calculateCostPerCal(&Data.productPrice, &totalCalories, &Data.caloriesCost_Per_Serving);
...
void displayReportData(const struct ReportData Data, const int cheapestProductCheck) {
	...
	printf("%07d %10.2lf %10.1lf %10.4lf %9d %8d %8.1lf %7.2lf %7.5lf", Data.skuNumber, Data.productPrice, Data.productWeightPounds, Data.productWeightKilos, Data.productWeightGrams, Data.Calories, Data.servings, Data.cost_Per_Serving, Data.caloriesCost_Per_Serving);
}
...
void start(void)
{
	...
	double cheapestProductPrice = Data[0].cost_Per_Serving;
	for (j = 0; j < Max_Products; j++)
	{
		if (Data[j].cost_Per_Serving <= cheapestProductPrice)
		{
			cheapestProductPrice = Data[j].cost_Per_Serving;
			cheapestIndex = j;
		}
	}
	printf("\n");
